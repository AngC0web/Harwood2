...
$(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success == 1) {
                            $('.error-code').html(response.message).attr('class', 'alert alert-success').show();
                        } else {
                            $('.error-code').html(response.message).attr('class', 'alert alert-danger').show();
                        }
                    },
                    error: function(xhr, status, error) {
                        $('.error-code').html('An error occurred').attr('class', 'alert alert-danger').show();
                    }
                });
            });
        });
    ...
    lazyImage();
    $('[name="with_broker"]').on('change', function() {
        var withBroker = $(this).val();
        if (withBroker == 1) {
            $('#broker_no').prop('checked', true);
        }
    });
    $('[name="broker"]').on('change', function() {
        var Broker = $(this).val();
        if (Broker == 1) {
            $('#with_broker_no').prop('checked', true);
        }
    });
    $('[name="broker"], [name="with_broker"]').on('change', function() {
        var withBroker = $('[name="with_broker"]:checked').val();
        var isAgent = $('[name="broker"]:checked').val();
        if (isAgent == 1 || withBroker == 1) {
            $('.brokerage').addClass('show');
            $('[name="brokerage"]').attr('required', true);
        } else {
            $('.brokerage').removeClass('show');
            $('[name="brokerage"]').attr('required', false);
        }
    });

function lazyImage() {
    $('[bg-src]').each(function() {
        var bg = $(this).attr('bg-src');
        $(this).css('background-image', 'url("' + bg + '")');
    });
    $('[image-src]').each(function() {
        var image = $(this).attr('image-src');
        if ($(window).width() < 768) {
            if ($(this).attr('image-msrc')) {
                image = $(this).attr('image-msrc');
            }
        }
        $(this).attr('src', image);
    });
